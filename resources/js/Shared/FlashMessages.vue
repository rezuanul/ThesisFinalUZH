<template>
  <div>
    ...
    <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" v-for="error in $page.props.errors" class="mb-8 flex items-center justify-between bg-red-500 rounded max-w-3xl">
      <div class="flex items-center">
...
export default {
  ...
  computed: {
    displayError: function() {
        ...
        let keys = Object.keys(this.$page.props.errors);

        if (keys.length > 0) {
          if (keys.length > 1) {
            message = 'There is one or more errors on page, please resolve to proceed.'
          } else if (keys.length > 0) {
            message = this.$page.props.errors[keys[0]]
          }
